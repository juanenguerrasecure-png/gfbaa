...
export function GlobalManualSaleModal({ isOpen, onClose }) {
  ...
  const handleBatchSelect = (batchId) => {
    ...
    if (batch) {
      ...
      if (linkedCatalog && linkedCatalog.price) {
        // If in bulk mode, set default price to unit price * 1, otherwise unit price
        setPriceInput(String(linkedCatalog.price));
      } else {
        setPriceInput('');
      }
      setQty(1);
  ...
  const handleQtyChange = (val) => {
    ...
    // Scale bulk price automatically when quantity changes
      ...
      >
        ...
        <div className="flex items-center justify-between px-6 py-4 bg-stone-900 text-white">
          <div className="flex items-center gap-2">
            ...
            <h3 className="font-display text-lg font-semibold tracking-wide">Direct / Manual Sale Entry</h3>
          </div>
          <button 
        ...
        <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-6 space-y-5">
          ...
          {/* Form Message Info */}
          <div className="text-xs text-stone-500 bg-stone-50 p-3 rounded border border-stone-200">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              ...
              <div className="flex flex-col gap-1">
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full pl-8 pr-3 py-1.5 border border-stone-300 rounded text-xs bg-white h-9"
                    value={search}
                  ...
                  {search && (
                    <button 
                      type="button" 
                      onClick={() => setSearch('')}
                      className="absolute right-2.5 top-2.5 text-stone-400 hover:text-stone-700 text-[10px] font-bold"
                    >
                      Clear
                    </button>
                  )}
                ...
                {/* Batch list */}
                <div 
                  className="mt-1.5 border border-stone-200 rounded overflow-y-auto bg-stone-50 p-1.5 space-y-1"
                  style={{ height: '180px' }}
                  id="global_sale_batch_list"
                >
                    ...
                    filteredBatches.map(b => {
                      ...
                      return (
                        <button
                          ...
                          className={`w-full text-left p-2 rounded border transition-all flex items-center justify-between text-xs ${
                            isSelected 
                        ...
                        >
                          <div className="flex-1 pr-2 truncate">
                            <div className="flex items-center gap-1.5">
                              <span className={`px-1 rounded font-mono text-[9px] ${isSelected ? 'bg-amber-500 text-stone-950' : 'bg-stone-100 text-stone-600'}`}>
                                {b.batchNumber}
                              </span>
                          ...
                          <div className="text-right flex-shrink-0">
                            <span className={`font-semibold ${isSelected ? 'text-amber-400' : 'text-emerald-700'}`}>
                              {b.remainingQty} left
                            </span>
            ...
            <div className="space-y-3">
              ...
              <div className="flex flex-col gap-1">
                ...
                <div className="flex items-center gap-2">
                  <input
                    id="global_sale_qty_input"
                    type="number"
                    min="1"
                    max={selectedBatch ? selectedBatch.remainingQty : 1}
                    value={qty}
                    onChange={e => handleQtyChange(e.target.value)}
                    disabled={!selectedBatchId}
                    className="w-full p-2 border border-stone-300 rounded text-xs bg-white disabled:opacity-50 disabled:bg-stone-100 h-9"
                    required
                  />
                  {selectedBatch && (
                    <button
              ...
              <div className="flex flex-col gap-1">
                ...
                <div className="grid grid-cols-2 gap-1.5 bg-stone-100 p-1 rounded border border-stone-200">
                  <button
                    ...
                    className={`py-1 text-[11px] font-semibold rounded text-center transition-all ${
                      priceType === 'unit' 
              ...
              {/* Price input */}
              <div className="flex flex-col gap-1">
                <label className="text-[10px] font-bold uppercase text-stone-500 tracking-wider">
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    disabled={!selectedBatchId}
                    className="w-full pl-7 pr-3 py-1.5 border border-stone-300 rounded text-xs bg-white disabled:opacity-50 disabled:bg-stone-100 h-9"
                    required
                  />
                </div>
                {selectedBatch && (
                  <div className="flex justify-between text-[10px] text-stone-400 mt-0.5">
                    ...
                    {priceType === 'bulk' && priceInput && (
                      <span className="font-semibold text-stone-500">
                        Computed: ${(Number(priceInput) / qty).toFixed(2)}/ea
                      </span>
                    )}
                  </div>
                )}
              ...
              <div className="flex flex-col gap-1">
                ...
                <div className="relative">
                  ...
                  <input
                    id="global_sale_buyer_input"
                    type="text"
                    placeholder="e.g. In-person client"
                    value={buyer}
                    onChange={e => setBuyer(e.target.value)}
                    className="w-full pl-7 pr-3 py-1.5 border border-stone-300 rounded text-xs bg-white h-9"
                  />
                </div>
              </div>
          ...
          {selectedBatch && (
            <div className="bg-stone-950 text-stone-100 p-4 rounded-lg border border-stone-800 font-mono text-xs space-y-2 animate-fade-in" id="global_sale_financials">
              <div className="flex justify-between">
                <span className="text-stone-400">Total Items Sold:</span>
                <span className="text-white font-semibold">{qty} units</span>
              </div>
              <div className="flex justify-between">
                <span className="text-stone-400">Total Revenue:</span>
                <span className="text-white font-semibold">${currentRevenue.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
              </div>
              ...
              <div className="flex justify-between border-t border-stone-800 pt-2 font-semibold text-sm text-emerald-400">
                <span>Estimated Direct Profit:</span>
          ...
          {/* Messages */}
          {error && (
            <div className="text-xs font-semibold text-red-600 bg-red-50 p-2.5 rounded border border-red-200" id="global_sale_error">
              ⚠️ {error}
            </div>
          )}

          {success && (
            <div className="text-xs font-semibold text-green-700 bg-green-50 p-2.5 rounded border border-green-200 flex items-center gap-1.5" id="global_sale_success">
