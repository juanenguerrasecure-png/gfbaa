    ...
    <div className={styles.wrap}>
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center border-b border-stone-200 mb-5 gap-4 pb-2 sm:pb-0">
        <div className="flex -mb-px">
          <button
            ...
            className={`px-5 py-2.5 text-xs font-semibold uppercase tracking-wider transition-all border-b-2 ${activeSubView === 'catalog' ? 'border-[#C9A84C] text-stone-900 font-bold' : 'border-transparent text-stone-500 hover:text-stone-700'}`}
          >
            Product Catalog Listings ({catalogItems.length})
          </button>
          <button
            onClick={() => setActiveSubView('batches')}
            className={`px-5 py-2.5 text-xs font-semibold uppercase tracking-wider transition-all border-b-2 ${activeSubView === 'batches' ? 'border-[#C9A84C] text-stone-900 font-bold' : 'border-transparent text-stone-500 hover:text-stone-700'}`}
          >
              ...
              {filteredCatalogItems.map(item => {
                ...
                return (
                  <tr key={item.id}>
                    ...
                    <td>
                      <span className={`px-2.5 py-1 text-xs font-bold rounded-full ${stock > 0 ? 'bg-emerald-50 text-emerald-800 border border-emerald-100' : 'bg-stone-100 text-stone-400'}`}>{stock} available</span>
                    </td>
                    ...
                    <td>
                      <div className={styles.actions}>
                        ...
                        <button
                          className={`${styles.iconBtn} ${styles.sellBtn} bg-stone-900 text-white`}
                          title="Record Sale"
                          aria-label={`Record sale for ${item.brand} ${item.name}`}
                          disabled={stock <= 0}
                          style={{ opacity: stock <= 0 ? 0.4 : 1 }}
                          onClick={() => openManualSale(item.batchId || item.id, `${item.brand} ${item.name}`)}
                        >
                          <ShoppingBag size={13} strokeWidth={1.8} />
                        </button>
                        <button className={`${styles.iconBtn} ${styles.deleteBtn}`} title="Unlist Catalog Item" onClick={() => {
                          if (confirm(`Are you sure you want to unlist this item "${item.name}" from the storefront? The acquisition batch records and remaining stock will NOT be deleted.`)) deleteCatalogItem(item.id);
                        }}>
                ...
                <tr>
                  <td colSpan={6}>
                    <div className="flex flex-col items-center justify-center text-center py-12 px-4 bg-stone-50/50 border border-dashed border-stone-200 rounded-lg m-4" id="empty_catalog_state">
                      ...
                      <p className="text-stone-500 text-xs max-w-sm mb-5 font-sans">You have not created any retail listing pages for the storefront yet. Direct your buyers to listed items or create one now.</p>
                      <button onClick={() => onSwitchTab && onSwitchTab('add')} className="px-4 py-2 bg-stone-900 hover:bg-stone-800 text-stone-100 text-xs font-semibold rounded shadow-sm transition-all flex items-center gap-1.5 cursor-pointer" id="empty_catalog_add_btn">
                        <PlusCircle size={13} />
                    ...
                    <td>
                      <div className="flex gap-1.5 justify-center">
                        <button
                          ...
                          title="Record Sale"
                          aria-label={`Record sale for ${prod.brand} ${prod.name}`}
                          disabled={batch.remainingQty <= 0}
                          style={{ opacity: batch.remainingQty <= 0 ? 0.4 : 1 }}
                          onClick={() => openManualSale(batch.id, `${prod.brand} ${prod.name}`)}
                        >
                          <ShoppingBag size={13} strokeWidth={1.8} />
                        </button>
                ...
                <tr>
                  <td colSpan={9}>
                    <div className="flex flex-col items-center justify-center text-center py-12 px-4 bg-stone-50/50 border border-dashed border-stone-200 rounded-lg m-4" id="empty_batches_state">
                      ...
                      <p className="text-stone-500 text-xs max-w-sm mb-5 font-sans">To manage your gold jewelry or pre-loved designer bags inventory, register a wholesale acquisition batch first.</p>
                      <button onClick={() => onSwitchTab && onSwitchTab('add')} className="px-4 py-2 bg-[#C9A84C] hover:bg-[#b7963d] text-stone-950 text-xs font-semibold rounded shadow-sm transition-all flex items-center gap-1.5 cursor-pointer" id="empty_batches_add_btn">
                        <PlusCircle size={13} />
      ...
      {manualSaleTarget && (
        <GlobalManualSaleModal
          isOpen={Boolean(manualSaleTarget)}
          onClose={() => setManualSaleTarget(null)}
          initialItemId={manualSaleTarget.id}
          initialItemName={manualSaleTarget.name}
        />
      )}
