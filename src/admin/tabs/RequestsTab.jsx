        ...
        return (
          <div id="print_area_backdrop" className="fixed inset-0 bg-stone-900/50 backdrop-blur-3xs z-50 flex items-start justify-center p-4 overflow-y-auto" onClick={() => setInvoicePreviewRequest(null)}>
            <div className="bg-white rounded w-full max-w-2xl p-8 space-y-8 shadow-2xl border border-stone-200 font-sans my-4 md:my-8" id="print_area" onClick={e => e.stopPropagation()}>
              ...
              {/* PAGINATED INVOICE SECTIONS */}
              {(() => {
                const allItems = invoicePreviewRequest.items || [];
                const hasMultiple = allItems.length > 1;
                const itemsPerPage = hasMultiple ? 3 : 4;
                const itemChunks = [];
                ...
                const totalItemPages = itemChunks.length;
                const companionPageNum = totalItemPages + 1;

                return (
                  <>
                    {itemChunks.map((chunk, chunkIdx) => {
                      const isFirstPage = chunkIdx === 0;
                      const isLastPage = chunkIdx === totalItemPages - 1;
                      ...
                      return (
                        <Fragment key={chunkIdx}>
                          {chunkIdx > 0 && (
                            <div className="page-break my-10 border-t border-dashed border-stone-300 relative">
                              <span className="absolute left-1/2 -top-3 -translate-x-1/2 bg-stone-100 border border-stone-200 text-stone-500 text-[10px] font-bold px-3 py-1 rounded-full uppercase tracking-wider print:hidden">
                                Page {pageNumber} of {companionPageNum}
                              </span>
                            </div>
                          )}
                          <div className="space-y-6 avoid-break invoice-page print:border-t-2 print:border-b-2 print:border-stone-900 print:py-8 print:my-6">
                            {/* Header */}
                          {isFirstPage ? (
                            /* Main Invoice Header */
                            ...
                            <div className="flex justify-between items-center border-b-2 border-stone-900 pb-3">
                              <div>
                                <h3 className="font-serif text-lg font-light text-stone-950 tracking-tight">GOOD FINDS<span className="text-[#C9A84C] font-normal"> by AA</span></h3>
                                <span className="text-[8px] uppercase font-semibold tracking-widest text-stone-400 block">Luxury Resale & Curation Invoice — Continued</span>
                              </div>
                              <div className="text-right">
                                <span className="font-mono text-2xs text-stone-700 block font-semibold">INVOICE NO: #{invoicePreviewRequest.requestNumber || invoicePreviewRequest.id.toString().slice(-6)}</span>
                                <span className="text-[9px] text-[#C9A84C] block font-semibold">Page {pageNumber} of {companionPageNum}</span>
                              </div>
                            </div>
                          )}

                          {/* Billing and Payment Meta Details (Page 1 Only) */}
                          {isFirstPage && (
                              ...
                              {chunk.map((item, idx) => {
                                ...
                                return (
                                  <tr key={idx} className="text-stone-800">
                                    <td className="py-4 align-middle font-medium text-stone-900">
                                      <div className="flex items-center gap-4">
                                        {photoUrl ? (
                                          <img 
                                            src={photoUrl} 
                                            alt={item.name} 
                                            className="w-28 h-28 md:w-32 md:h-32 rounded-lg object-cover border border-stone-200 shrink-0 bg-stone-50 shadow-xs"
                                            referrerPolicy="no-referrer"
                                          />
                                        ) : (
                                          <div className="w-28 h-28 md:w-32 md:h-32 rounded-lg bg-stone-100 border border-stone-200 flex items-center justify-center text-stone-400 shrink-0 shadow-xs">
                                            <ShoppingBag size={28} />
                                          </div>
                                        )}
                                        <div className="self-center">
                                          <span className="block font-medium text-stone-900 text-sm sm:text-base leading-snug">{item.name}</span>
                                    ...
                                    <td className="py-4 align-middle text-right uppercase tracking-wider text-[10px] text-stone-500 font-medium">{item.brand}</td>
                                    <td className="py-4 align-middle text-right font-mono font-medium text-stone-600">{item.qty || 1}</td>
                                    <td className="py-4 align-middle text-right font-mono font-bold text-stone-950">${Number(item.price).toLocaleString()}</td>
                                  </tr>
                                );
                              })}
                            </tbody>
                          ...
                          {/* Pricing breakdown and payment methods (Only on Last Items Page) */}
                          {isLastPage ? (
                            <>
                              ...
                              <div className="text-[9px] text-stone-400 leading-relaxed border-t border-stone-100 pt-4 font-sans text-center">
                                <span>Thank you for choosing Good Finds by AA. To complete settlement, please scan the large QR code above or send payments to the specified handler account. Curation reservations are automatically released if left unpaid. Please see Page {companionPageNum} for complete buyer steps.</span>
                              </div>
                            </>
                          ) : (
                            <div className="text-[10px] text-stone-400 leading-relaxed border-t border-stone-100 pt-3 font-sans text-right italic font-medium">
                              <span>Invoice continued on Page {pageNumber + 1}...</span>
                            </div>
                          )}
                        </div>
                      </Fragment>
                    ...
                    <div className="page-break my-10 border-t border-dashed border-stone-300 relative">
                      <span className="absolute left-1/2 -top-3 -translate-x-1/2 bg-stone-100 border border-stone-200 text-stone-500 text-[10px] font-bold px-3 py-1 rounded-full uppercase tracking-wider print:hidden">
                        Page {companionPageNum} (Transaction & Shipping Steps)
                      </span>
                    </div>

                    {/* DYNAMIC COMPANION PAGE: CLIENT TRANSACTION & CURATION GUIDE */}
                    <div className="space-y-6 pt-4 print:pt-8 print:my-6 avoid-break invoice-page print:border-t-2 print:border-b-2 print:border-stone-900">
                      {/* Header for Companion Page */}
                      <div className="flex justify-between items-start border-b-2 border-stone-900 pb-5">
                        <div>
                        ...
                        <div className="space-y-3 font-sans">
                          ...
                          <div className="flex gap-4 p-3.5 bg-stone-50 rounded border border-stone-200/60 items-start">
                            ...
                            <div className="space-y-0.5">
                              ...
                              <p className="text-2xs text-stone-500 leading-relaxed">
                                Scan the large custom QR code on Page {totalItemPages} or copy the account handler details exactly. Submit your total outstanding balance using your choice of Zelle, Venmo, or PayPal within the 24-hour guarantee window.
                              </p>
                            </div>
                          </div>
