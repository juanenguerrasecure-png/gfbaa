  ...
  return (
    <div className={styles.overlay} onClick={onClose}>
      <section className={styles.modal} onClick={event => event.stopPropagation()} aria-modal="true" role="dialog" aria-label={`${product.brand} ${product.name}`}>
        <button type="button" className={styles.closeBtn} onClick={onClose} aria-label="Close product details">
          <X size={20} />
        </button>
        ...
        <div className={styles.gallery} onTouchStart={event => setTouchStart(event.touches[0].clientX)} onTouchEnd={handleTouchEnd}>
          {photos.length ? (
            ...
            <div className={styles.fallbackPhoto}>{product.emoji || FALLBACK_EMOJI[product.cat] || 'GF'}</div>
          )}

          {photos.length > 1 && (
            <>
              <button type="button" className={`${styles.arrow} ${styles.arrowLeft}`} onClick={goPrev} aria-label="Previous photo"><ChevronLeft size={20} /></button>
              <button type="button" className={`${styles.arrow} ${styles.arrowRight}`} onClick={goNext} aria-label="Next photo"><ChevronRight size={20} /></button>
              <div className={styles.dots}>{photos.map((_, index) => <button key={index} type="button" className={`${styles.dot} ${index === activePhoto ? styles.activeDot : ''}`} onClick={() => setActivePhoto(index)} aria-label={`Show photo ${index + 1}`} />)}</div>
            </>
          )}
        </div>
        ...
        <div className={styles.infoPanel}>
          ...
          <div className={styles.metaRow}>
            ...
            <span className={styles.category}>{product.cat || 'curation'}</span>
          </div>

          <div className={styles.priceRow}>
            <span className={styles.price}>{formatProductPrice(product, currency)}</span>
            <PriceToggle compact />
          </div>
        ...
        <div className={styles.actionBar}>
          <button type="button" className={styles.addBtn} onClick={handleAdd} disabled={soldOut}>
            <ShoppingBag size={16} />
            {soldOut ? 'Sold Out' : 'Add to Cart'}
          </button>
          {whatsappHref && (
            <a className={styles.whatsAppBtn} href={whatsappHref} target="_blank" rel="noopener noreferrer">
